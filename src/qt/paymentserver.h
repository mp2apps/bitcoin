...
public:
    ...
    PaymentServer(QObject* parent=NULL, bool startLocalServer=true); // parent should be QApplication object
    ~PaymentServer();
...
private:
    static bool ReadPaymentRequest(const QString& filename, SendCoinsRecipient& recipient);
    static bool processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient);
