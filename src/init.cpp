...
void Shutdown()
{
    ...
    pwalletBackend->ShutdownPreNodeStop();
    StopNode();
    ...
    pwalletBackend->ShutdownPostNodeStop();
    boost::filesystem::remove(GetPidFile());
    UnregisterAllWallets();
    pwalletBackend->DeleteWallets();
...
/** "Wallet backend" that simply ignores all calls */
class CDummyWalletBackend: public CWalletBackendInterface
{
    bool ParseAndValidateArguments() { return true; }
    bool VerifyWallets(std::ostringstream &strErrors) { return true; }
    ...
    const CKeyStore *GetKeystore() { return 0; }
};
CDummyWalletBackend dummyWalletBackend;
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    pwalletBackend = &dummyWalletBackend;
#ifdef ENABLE_WALLET
    ...
    if(!pwalletBackend->ParseAndValidateArguments())
        return false;
    ...
    if(!pwalletBackend->VerifyWallets(strErrors))
        return false;
    ...
    // ********************************************************* Step 8: load wallet
    if(!pwalletBackend->LoadWallets(strErrors))
        return false;
    ...
    pwalletBackend->ShowStartupStatistics();
    ...
    pwalletBackend->GenerateBitcoins(GetBoolArg("-gen", false), GetArg("-genproclimit", -1));

    // ********************************************************* Step 12: finished

    uiInterface.InitMessage(_("Done loading"));

    pwalletBackend->InitializePostNodeStart(threadGroup);
